...
- làm thế nào một con thiết bị biết được thiết bị đấy của hãng của hãng hay ko thì nó dựa vào chuỗi public key, private key -> gọi là kỹ thuật khóa 1 chiều, tư tưởng như sau:
  ...
  - boot rom, sau khi đọc image từ sdcard, sẽ verify chữ ký bằng public key. Public key được lưu ở trên boot rom.
...
- Nếu chữ ký trùng khớp với `public key` thì `boot rom` mới `load image` lên.
...
Boot ROM về cơ bản rất là nhẹ tại vì:
- vùng nhớ lưu trữ nó cũng bé.
- code của nó giới hạn rất nhiều tính năng của ngôn ngữ lập trình.

Nên người ta không muốn làm nó phức tạp.
bản chất của boot ROM nhằm đảm bảo tính năng của secure boot và cấu hình được hardware để có thể đọc từ MMC hoặc sdcard.

Ngày nay để load được kernel của các hệ điều hành lên thường rất là phức tạp vì bản thân kernel của hệ điều hành phải giải mã vì đa số kernel image đều ở dạng mã hóa hoặc là dạng nén. Thế nên, khiến cho boot rom không thể nào thực hiện boot được kernel lên mà cần chương trình bootloader tức là boot rom nó cũng là bootloader mà nó rất đơn giản => sinh ra một cơ chế trung gian.

Bình thường, boot rom sẽ load kernel nhưng mà như chúng ta biết để load kernel nó rất phức tạp chứ không giản nên chúng ta cần một bộ code trung gian phức tạp hơn đúng ở giữa thì nó chính là `u-boot`. Như vậy `boot rom` sẽ load `u-boot` và `u-boot` sẽ load kernel do u-boot được lưu ở bộ nhớ ngoài, nên không bị giới hạn về mặt kích thước source code có thể thực thi công việc phức tạp.


CPU load boot rom tại địa chỉ 0x20000. Boot rom sẽ load uboot tại offset 128000 của thiết bị lưu trữ. U-boot sẽ load kernel image thông qua đường dẫn file.

Quá trình khởi động để load được linux kernel lên, được chia làm 3 giai đoạn:
- 1st stage: số càng nhỏ thì nó càng gần với thời điểm cpu được bật lên, tài nguyên sử dụng càng ít. (cpu vừa được bật lên chỉ dùng được mỗi thanh ghi của nó thôi chứ chưa dùng được RAM).
- 2nd stage
- 3rd stage

cả 2nd và 3rd đều là code của `uboot` trong đó:
- 2nd: SPL/MLO
- 3rd: u-boot-dtb.img

Tại sao lại phân chia MLO và u-boot-dtb.img? Vấn đề là khi boot rom load u-boot lên thì boot rom nó mới khởi tạo được SRAM thôi thì SRAM chỉ đủ chạy một thằng rất là nhỏ vì vậy người ta phân chia MLO và u-boot-dtb.img vì MLO nhỏ đủ chạy trong SRAM. MLO khởi tạo SRAM rồi sẽ load u-boot-dtb.img.

> Lưu ý: SRAM là RAM nằm trong chính CPU luôn còn DRAM là chip RAM nằm ở ngoài ở trên bo mạch.
